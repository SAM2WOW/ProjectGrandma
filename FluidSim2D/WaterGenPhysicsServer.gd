...
func create_particle():
	...
	vs.canvas_item_set_parent(water_particle, GameManager.instantiationManager.get_canvas_item())
	#set its transform
	vs.canvas_item_set_transform(water_particle, GameManager.instantiationManager.global_transform)
	...
	vs.canvas_item_set_self_modulate(water_particle,GameManager.instantiationManager.liquidColors[liquid_type]);
	# vs.canvas_item_set_z_index(water_particle,2);
	#add RID pair to array
	water_particles.append([water_col,water_particle])
	if (!GameManager.instantiationManager.liquidParticles.keys().has(liquid_type)):
		GameManager.instantiationManager.liquidParticles[liquid_type] = {};
	GameManager.instantiationManager.liquidParticles[liquid_type][water_col] = LiquidState.new(
		liquid_type, water_col, water_particle, GameManager.instantiationManager.liquidColors[liquid_type]
	);
